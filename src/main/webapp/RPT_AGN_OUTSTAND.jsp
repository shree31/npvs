...
<%	
	Logger logger = Logger.getLogger("RPT_AGN_OUTSTAND");

	HashMap RPT_AGN_OUTSTANDObj = (HashMap)request.getAttribute("RESPONSE");

	
	if(RPT_AGN_OUTSTANDObj == null)
  		RPT_AGN_OUTSTANDObj = new HashMap();

	String formName = "RPT_AGN_OUTSTAND";

%>


<TITLE id="RPT_AGN_OUTSTAND_title"><%= field_labels.getString("RPT_AGN_OUTSTAND.RPT_AGN_OUTSTAND_title") %></TITLE>
...
{
	if(as_field_name.name=="FROM_DATE")
	{
			checkdate(as_field_name);

//	if((as_field_name.name=="FROM_DATE") || (as_field_name.name=="TO_DATE"))
//		doDateCheck(document.getElementById("FROM_DATE"),document.getElementById("TO_DATE"));
	}
...
function preSubmit()
{
	createParamStr('CMP_CODE,CNT_CODE,AGN_CODE,CATEGORY_CODE,FROM_DATE');
...
<DIV id="RPT_AGN_OUTSTAND_div" >

<FORM name="RPT_AGN_OUTSTAND" id="RPT_AGN_OUTSTAND" method="post" action="reortGenToolActionServlet.do">
<INPUT type="hidden" id="Event" name="Event" value="">
<INPUT type="hidden" id="MAKER" name="MAKER" value="<%= (String)RPT_AGN_OUTSTANDObj.get("MAKER") %>">
<INPUT type="hidden" id="MAKER_DT" name="MAKER_DT" value="">
<INPUT type="hidden" id="STATUS" name="STATUS" value="<%= (String)RPT_AGN_OUTSTANDObj.get("STATUS") %>">
<INPUT type="hidden" id="MODE" name="MODE" value="INP">
<INPUT type="hidden" id="ENTITY" name="ENTITY" value="RPT_AGN_OUTSTAND">
<INPUT type="hidden" id="PARAM" name="PARAM" value="<%= (String)RPT_AGN_OUTSTANDObj.get("PARAM") %>">
<INPUT type="hidden" id="PARAM1" name="PARAM1" value="<%= (String)RPT_AGN_OUTSTANDObj.get("PARAM1") %>">
<INPUT type="hidden" id="PARAM2" name="PARAM2" value="<%= (String)RPT_AGN_OUTSTANDObj.get("PARAM2") %>">
<INPUT type="hidden" id="PRIMARY_KEY" name="PRIMARY_KEY" value="">
<INPUT type="hidden" id="SCREEN_NAME" name="SCREEN_NAME" value="RPT_LINK">
<INPUT type="hidden" id="ERRDESC" name="ERRDESC" value="<%= (String)RPT_AGN_OUTSTANDObj.get("ERRDESC") %>">
<INPUT type="hidden" id="PROCESS_NAME" name="PROCESS_NAME" value="RPT_AGN_OUTSTAND">
<INPUT type="hidden" id="RPT_ID" name="RPT_ID" value="RP_AW_001">

    <TABLE class="headertable" cellspacing="0" cellpadding="0" width='100%'>
         <TD>&nbsp;</TD>
        <TR> 
           <TD class="heading" nowrap align="center">
            <%= field_labels.getString("RPT_AGN_OUTSTAND.RPT_AGN_OUTSTAND") %>
           </TD>
           <TD>&nbsp;</TD>
        </TR>
   </TABLE>
 ...
 <TABLE class="detailstable" cellspacing="0" cellpadding="0">
	...
	<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="CMP_CODE_label" class="userlabel"><%= field_labels.getString("RPT_AGN_OUTSTAND.CMP_CODE") %><SPAN class="instreditalic">*</SPAN></LABEL>
		</TD>
		<TD class="ste" colspan="7" nowrap>
			<INPUT type="text" id="CMP_CODE" name="CMP_CODE" class="input" size="10" onChange="FieldValidation(this);getCodeDesc('CMP_CODE','CMP_CODEDESC', 'g_pkl_companyDesc','');" maxLength="10"  value="<%= (String)RPT_AGN_OUTSTANDObj.get("CMP_CODE") %>" >

	  <INPUT type="text" id="CMP_CODEDESC" name="CMP_CODEDESC" class="input" size="50" readonly  maxLength="50" value="<%= 
	(String)RPT_AGN_OUTSTANDObj.get("CMP_CODEDESC") %>" >
		...
		<TD>
				<INPUT type="text" id="CNT_CODE" name="CNT_CODE" class="input" size="10" maxLength="10"  value="<%= (String)RPT_AGN_OUTSTANDObj.get("CNT_CODE") %>"
			onChange="FieldValidation(this);getCodeDesc('CNT_CODE,USER_ID','CNT_CODEDESC', 'g_pkl_centerDesc','MC.CNT_CODE,USER_ID');" >
		  <INPUT type="text" id="CNT_CODEDESC" name="CNT_CODEDESC" class="input" size="50"  readonly maxLength="50"  value="<%= (String)RPT_AGN_OUTSTANDObj.get("CNT_CODEDESC") %>">
	...
	<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="FROM_DATE_label" class="userlabel"><%= field_labels.getString("RPT_AGN_OUTSTAND.FROM_DATE") %><SPAN class="instreditalic">*</SPAN></LABEL>
		</TD>
		<TD class="ste" colspan="7" nowrap>
			<INPUT type="text" id="FROM_DATE" name="FROM_DATE" class="input" size="10" onblur="FieldValidation(this);" maxLength="10"  value="<%= (String)RPT_AGN_OUTSTANDObj.get("FROM_DATE") %>" ><a href="#" onClick="javascript:calendar('FROM_DATE',event.screenY,event.screenX,null,null,getDatePickerPattern());"
...
<!--		<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="EDT_CODE_label" class="userlabel"><%= field_labels.getString("RPT_AGN_OUTSTAND.EDT_CODE") %><SPAN class="instreditalic">*</SPAN></LABEL>
		</TD>
		<TD class="ste" colspan="7" nowrap>
			<INPUT type="text" id="EDT_CODE" name="EDT_CODE" class="input" size="30"  onChange="FieldValidation(this);getCodeDesc('EDT_CODE,CMP_CODE,PUB_CODE','EDT_CODEDESC', 'g_pkl_editionDesc','');" maxLength="30"  value="<%= (String)RPT_AGN_OUTSTANDObj.get("EDT_CODE") %>" >
	  <INPUT type="text" id="EDT_CODEDESC" name="EDT_CODEDESC" class="input" size="50" readonly  maxLength="50" value="<%= 
	(String)RPT_AGN_OUTSTANDObj.get("EDT_CODEDESC") %>" >
	...
	<TR class="ste">
	        <TD class="ste" nowrap>   
				<LABEL ID="AGN_CODE_label" class="userlabel"><%= field_labels.getString("RPT_AGN_OUTSTAND.AGN_CODE") %></LABEL>
		
		</TD>
		<TD>
				<INPUT type="text" id="AGN_CODE" name="AGN_CODE" class="input" size="10" maxLength="10"  value=""
			onChange="FieldValidation(this);getCodeDesc('AGN_CODE,CMP_CODE,USER_ID','AGN_NAME', 'g_pkl_agencyDesc','');" >
...
<!--	<TR class="ste">
	        <TD class="ste" nowrap>   
				<LABEL ID="ROUTE_CODE_label" class="userlabel"><%= field_labels.getString("RPT_AGN_OUTSTAND.ROUTE_CODE") %></LABEL>
		...
		<TR class="ste">
	        <TD class="ste" nowrap>   
				<LABEL ID="CATEGORY_CODE_label" class="userlabel"><%= field_labels.getString("RPT_AGN_OUTSTAND.CATEGORY_CODE") %></LABEL>
