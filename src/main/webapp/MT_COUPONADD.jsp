...
<%
	Logger logger = Logger.getLogger("MT_COUPONADD");
...
<%	
	...
	HashMap MT_COUPONADDObj = (HashMap)request.getAttribute("RESPONSE");
	
	if(MT_COUPONADDObj == null)
  		MT_COUPONADDObj = new HashMap();
	
	String formName = "MT_COUPONADD";
	...
	{
		l_str_query = obj_fetch_pick_vals.getQuery("MT_COUPONADD");		
		...
		l_str_query = l_str_query + " ORDER BY P.PUB_CODE";
...
<TITLE id="MT_COUPONADD_title"><%= field_labels.getString("MT_COUPONADD.MT_COUPONADD_title") %></TITLE>
...
function validateSubmit(as_field_name)
{
	if(chkMandatory()==true)
	{
		...
		document.location.href="MT_COUPONADD.jsp?AGN_CODE="+l_agency_code+"&PUB_CODE="+l_pub_code+"&CUST_ADDRESS="+l_cust_address+"&MONTH="+l_month+"&YEAR="+l_year;
//		document.parent.frames['COUPONADD'].href="MT_COUPONDETAILS.jsp";
//		document.frames['COUPONADD'].target="SEARCHDET";

//		document.location.href="MT_COUPONADD.jsp;
...
//	alert(url);
document.location.href="MT_COUPONDETAILS.jsp?AGN_CODE="+l_agency_code+"&PUB_CODE="+url+"&CUST_ADDRESS="+l_cust_address+"&MONTH="+l_month+"&YEAR="+l_year;
//	window.open("MT_COUPONDETAILS.jsp?AGN_CODE="+l_agency_code+"&PUB_CODE="+url+"&CUST_ADDRESS="+l_cust_address+"&MONTH="+l_month+"&YEAR="+l_year);
}
...
<DIV id="MT_COUPONADD_div" >

<FORM name="MT_COUPONADD" id="MT_COUPONADD" method="post" action="distAppCommonActionServlet.do">
<INPUT type="hidden" id="Event" name="Event" value="">
<INPUT type="hidden" id="MAKER" name="MAKER" value="<%= (String)MT_COUPONADDObj.get("MAKER") %>">
<INPUT type="hidden" id="MAKER_DT" name="MAKER_DT" value="">
<INPUT type="hidden" id="STATUS" name="STATUS" value="<%= (String)MT_COUPONADDObj.get("STATUS") %>">
<INPUT type="hidden" id="MODE" name="MODE" value="MOD">
<INPUT type="hidden" id="PARAM1" name="PARAM1" value="<%= (String)MT_COUPONADDObj.get("PARAM1") %>">
<INPUT type="hidden" id="PARAM2" name="PARAM2" value="<%= (String)MT_COUPONADDObj.get("PARAM2") %>">
<INPUT type="hidden" id="ENTITY" name="ENTITY" value="TR_PAY_HIS_DETAILS">
<INPUT type="hidden" id="SCREEN_NAME" name="SCREEN_NAME" value="MT_COUPONADD">
<INPUT type="hidden" id="ERRDESC" name="ERRDESC" value="<%= (String)MT_COUPONADDObj.get("ERRDESC") %>">
<INPUT type="hidden" id="PRIMARY_KEY" name="PRIMARY_KEY" value="AGN_CODE#BILL_GEN_DT,DATE">
<INPUT type="hidden" id="PARAMSTR" name="PARAMSTR" value="<%=(String)MT_COUPONADDObj.get("PARAMSTR") %>">

  <TABLE class="headertable" cellspacing="0" cellpadding="0" width='100%'>
         <TD>&nbsp;</TD>
        <TR> 
           <TD class="heading" nowrap align="center">
            <%= field_labels.getString("MT_COUPONADD.MT_COUPONADD") %>
           </TD>
        </TR>
   </TABLE>
 ...
 <TABLE class="detailstable" cellspacing="0" cellpadding="0">
	...
	<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="AGN_CODE_label" class="userlabel"><%= field_labels.getString("MT_COUPONADD.AGN_CODE") %><SPAN class="instreditalic">*</SPAN></LABEL>
		</TD>
	...
	<A id="AGN_CODEHLP" name="AGN_CODEEHLP"  href="#" onClick="getPicklistValues('g_pkl_agency','AGN_CODE','','')" class="linkblue" >?</A> &nbsp;
		...
		<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="YEAR_label" class="userlabel"><%= field_labels.getString("MT_COUPONADD.YEAR") %></LABEL>
		</TD>
		...
		<TD class="ste" nowrap>   
			<LABEL ID="MONTH_label" class="userlabel"><%= field_labels.getString("MT_COUPONADD.MONTH") %></LABEL>
		</TD>
		<TD class="ste" colspan="1" nowrap>
	...
	<TR>
		<TD class="ste" nowrap>   
			<LABEL ID="PUB_CODE_label" class="userlabel"><%= field_labels.getString("MT_COUPONADD.PUB_CODE") %></LABEL>
		</TD>
...
//	String l_passs_str = (String)l_inner_arr.get(0);
	String l_passs_str = "AGN_CODE="+l_agency_code+"&PUB_CODE="+(String)l_inner_arr.get(0)+"&CUST_ADDRESS="+l_cust_address+"&MONTH="+l_month+"&YEAR="+l_year;

%>
	<TR class="str">

		<TD class="ste" nowrap>   
			<A href="MT_COUPONDETAILS.jsp?<%=l_passs_str%>" id="Link" class="linkblue" onClick="OpenWindow('<%=l_passs_str%>')" ><%=(i+1)%></a>
		</TD>
