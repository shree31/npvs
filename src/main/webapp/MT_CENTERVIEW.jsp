...
{
	clearNull();
	var l_cnt_editn_flg = "<%=((String)MT_CENTERObj.get("CNT_EDITN_FLG"))%>";
	var l_cnt_distr_flg = "<%=((String)MT_CENTERObj.get("CNT_DISTR_FLG"))%>";
	var l_cnt_valid_flg = "<%=((String)MT_CENTERObj.get("CNT_VALID_FLG"))%>";
	if(l_cnt_editn_flg == "N")
		document.getElementById("CNT_EDITN_FLG_N").checked=true;
	else 
		document.getElementById("CNT_EDITN_FLG_Y").checked=true;
	if(l_cnt_distr_flg == "N")
		document.getElementById("CNT_DISTR_FLG_N").checked=true;
	else 
		document.getElementById("CNT_DISTR_FLG_Y").checked=true;
	if(l_cnt_valid_flg == "N")
		document.getElementById("CNT_VALID_FLG_N").checked=true;
	else 
		document.getElementById("CNT_VALID_FLG_Y").checked=true;
}
...
<INPUT type="hidden" id="SCREEN_NAME" name="SCREEN_NAME" value="MT_CENTERVIEW">
 ...
 <TABLE class="detailstable" cellspacing="0" cellpadding="0">
	...
	<TR class="ste">
		...
		<TD class="ste" colspan="2" nowrap>
			<INPUT type="text" id="CNT_CODE" name="CNT_CODE" class="input" size="10" maxLength="10" onChange="getCodeDesc('CNT_CODE','CNT_NAME','g_pkl_centerDesc','MC.CNT_CODE');" value="<%= (String)MT_CENTERObj.get("CNT_CODE") %>" >
				<A id="CNT_CODEHLP" name="CNT_CODEHLP"  href="#" onClick="getPicklistValues('g_pkl_center','CNT_CODE','','')" class="linkblue" >?</A> &nbsp;
		</TD>
		</TR>
		<TR class="ste">
		<TD class="ste" nowrap>   
	...
	<TR class="ste">
		...
		<TD class="ste" colspan="2" nowrap>
<!--			<INPUT type="text"  id="CMP_CODE" name="CMP_CODE" class="input" size="10" readonly maxlength="10" onChange="FieldValidation(this); getCodeDesc('CMP_CODE','CMP_CODEDESC', 'g_pkl_companyDesc','')"  value="<%= (String)MT_CENTERObj.get("CMP_CODE")%>"> -->
	<SELECT class="input"name='CMP_CODE' id="CMP_CODE" onChange="FieldValidation(this);getCodeDesc('CMP_CODE','CMP_CODEDESC', 'g_pkl_companyDesc','');"  onBlur="FieldValidation(this);" >
		<%
			l_selectedValue=(String)MT_CENTERObj.get("CMP_CODE");
		...
		%>
           	  ...
           	  <INPUT type="text" id="CMP_CODEDESC" name="CMP_CODEDESC" class="input" size="65" readonly  maxLength="65" value="<%= 
...
<!--		<A id="CMP_CODEHLP" name="CMP_CODEHLP"  href="#" onClick="getPicklistValues('g_pkl_company','CMP_CODE','','')" class="linkblue" >?</A> &nbsp; -->
	...
	<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="CITY_label" class="userlabel"><%= field_labels.getString("MT_CENTER.CITY") %></LABEL>
		</TD>
		<TD class="ste" colspan="2" nowrap>
			<INPUT type="text" id="CITY" name="CITY" class="input" size="10" readOnly onChange="FieldValidation(this);getCodeDesc( 'CITY','CITYDESC','g_pkl_cityDesc','KEY_CODE');" maxLength="10"  value="<%= (String)MT_CENTERObj.get("CITY") %>" >
	...
	<TR class="ste">
		...
		<INPUT type="radio" id="CNT_VALID_FLG_Y" name="CNT_VALID_FLG" checked onClick="FieldValidation(this);" value="Y" accessKey="Y" ><%= field_labels.getString("radio_yes") %>

		<INPUT type="radio" id="CNT_VALID_FLG_N" name="CNT_VALID_FLG"  onClick="FieldValidation(this);" value="N" accessKey="N" ><%= field_labels.getString("radio_no") %>
	...
	<TR class="ste">
		...
		<INPUT type="radio" id="CNT_DISTR_FLG_Y" name="CNT_DISTR_FLG" checked onClick="FieldValidation(this);" value="Y" accessKey="Y" ><%= field_labels.getString("radio_yes") %>

		<INPUT type="radio" id="CNT_DISTR_FLG_N" name="CNT_DISTR_FLG"  onClick="FieldValidation(this);" value="N" accessKey="N" ><%= field_labels.getString("radio_no") %>
		...
		<INPUT type="radio" id="CNT_EDITN_FLG_Y" name="CNT_EDITN_FLG" checked onClick="FieldValidation(this);" value="Y" accessKey="Y" ><%= field_labels.getString("radio_yes") %>

		<INPUT type="radio" id="CNT_EDITN_FLG_N" name="CNT_EDITN_FLG"  onClick="FieldValidation(this);" value="N" accessKey="N" ><%= field_labels.getString("radio_no") %>
