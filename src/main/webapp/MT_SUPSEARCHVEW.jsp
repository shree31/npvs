...
<%
	Logger logger = Logger.getLogger("MT_BASESUPSEARCHVIEW");
...
function validateSubmit(as_field_name)
{
	if(chkMandatory()==true)
	{
		...
		var l_agency_codedesc = document.getElementById("AGN_CODEDESC").value==""?"null":document.getElementById("AGN_CODEDESC").value.toUpperCase();
		...
		var l_CUST_ADDRESS = l_agency_codedesc;
		
		document.location.href="MT_BASESUPSEARCHRES.jsp?AGN_CODE="+l_agency_code+"&AGN_DESC="+l_agency_codedesc+"&PUB_CODE="+l_pub_code+"&CUST_ADDRESS="+l_CUST_ADDRESS;
...
<INPUT type="hidden" id="PARAMSTR" name="PARAMSTR" value="<%=(String)MT_AGNSUPSEARCHObj.get("PARAMSTR") %>">
 ...
 <TABLE class="detailstable" cellspacing="0" cellpadding="0">
	...
	<TR class="ste">
	  ...
	  <INPUT type="text" id="AGN_CODEDESC" name="AGN_CODEDESC" class="input" size="50" readonly  maxLength="500" value="" >
	<A id="AGN_CODEHLP" name="AGN_CODEEHLP"  href="#" onClick="getPicklistValues('g_pkl_agency','AGN_CODE','','')" class="linkblue" >?</A> &nbsp;
	  ...
	  <INPUT type="text" id="CUST_ADDRESSDESC" name="CUST_ADDRESSDESC" class="input" size="50"  maxLength="50" value="" >
		<A id="CUST_ADDRESSHLP" name="CUST_ADDRESSHLP"  href="#" onClick="getPicklistValues('g_pkl_Address','CUST_ADDRESS','','')" class="linkblue" >?</A> &nbsp;
 -->
	</TR>	
	<TR>
		<TD class="ste" nowrap>   
			<LABEL ID="PUB_CODE_label" class="userlabel"><%= field_labels.getString("MT_AGNSUPSEARCH.PUB_CODE") %></LABEL>
		</TD>
