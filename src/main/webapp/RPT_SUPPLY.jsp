...
function preSubmit()
{
	var sup_dt = document.getElementById("SUP_DT").value;
	if(sup_dt.substr(0,1) == "0")
		sup_dt = sup_dt.substr(1,1);
	else
		sup_dt = sup_dt.substr(0,2);
	
	document.getElementById("SUP_DAY").value = "DAY"+sup_dt;
	createParamStr('ROUTE_CODE,PUB_CODE,SUP_DT,SUP_DAY,LANGUAGE');
	validateSubmit();
}
...
<INPUT type="hidden" id="PROCESS_NAME" name="PROCESS_NAME" value="RPT_SUPPLY">
<INPUT type="hidden" id="RPT_ID" name="RPT_ID" value="RP_SU_001">
<INPUT type="hidden" id="SUP_DAY" name="SUP_DAY" value="">
   ...
   <TABLE class="headertable" cellspacing="0" cellpadding="0" width='100%'>
        ...
        <TR> 
           <TD class="heading" nowrap align="center">
            <%= field_labels.getString("RPT_ORDER.SUP_RPT") %>
           </TD>
           <TD>&nbsp;</TD>
        </TR>
   </TABLE>
 ...
 <TABLE class="detailstable" cellspacing="0" cellpadding="0">
	...
	<TR class="ste">
		...
		<TD class="ste" colspan="7" nowrap>
			<INPUT type="text" id="PUB_CODE" name="PUB_CODE" class="input" size="10"  onChange="FieldValidation(this);getCodeDesc('PUB_CODE','PUB_CODEDESC', 'g_pkl_publicationDesc','');" maxLength="10"  value="<%= (String)RPT_ORDERObj.get("PUB_CODE") %>" >
	...
	<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="LANGUAGE_label" class="userlabel"><%= field_labels.getString("RPT_ORDER.LANGUAGE") %><SPAN class="instreditalic">*</SPAN></LABEL>
		</TD>
		<TD class="ste" colspan="7" nowrap>
...
<!--  		<TR class="ste">
		<TD class="ste" nowrap>   
			<LABEL ID="SUP_DT_TO_label" class="userlabel"><%= field_labels.getString("RPT_ORDER.SUP_DT") %>  <SPAN class="instreditalic">*</SPAN></LABEL>
		</TD>
		<TD class="ste" colspan="7" nowrap>
			<INPUT type="text" id="SUP_DT_TO" name="SUP_DT_TO" class="input" size="10" onblur="FieldValidation(this);" maxLength="10"  value="<%= (String)RPT_ORDERObj.get("SUP_DT") %>" ><a href="#" onClick="javascript:calendar('SUP_DT',event.screenY,event.screenX,null,null,getDatePickerPattern());"
			title="Select Date"><img src="images/calendar.gif" width="16"
			height="16" border="0" /></a>
		</TD>
	</TR>-->
