...
function execute_onLoad()
{
	...
	var ob = document.all("MNU_LST");
	var l_stored_val = document.getElementById("TOP_MENU_LIST").value;
	var l_split_top_menu = l_stored_val.split(";");
	
	for(var j=0;j<l_split_top_menu.length;j++)
	{
		for (var i=0;i<ob.length;i++)
		{
			if (ob.options[i].value==l_split_top_menu[j]) 
			{
	...
	ob = document.all("CNT_LST");
	l_stored_val = document.getElementById("CNT_CODE_LST").value;
	l_split_top_menu = l_stored_val.split(";");
...
<INPUT type="hidden" id="SCREEN_NAME" name="SCREEN_NAME" value="MT_USERCRNVIEW">
...
<INPUT type="hidden" id="CNT_CODE_LST" name="CNT_CODE_LST" value="<%=(String)MT_USERCRNObj.get("CNT_CODE_LST")%>">
 ...
 <TABLE class="detailstable" cellspacing="0" cellpadding="0">
	...
	<TR class="ez1">
		...
		<TD class="ez1" colspan="2" nowrap>
			<INPUT type="text" id="USR_ID" name="USR_ID" class="input" size="15" onblur="FieldValidation(this);" onChange="FieldValidation(this);" maxLength="15"  value="<%= (String)MT_USERCRNObj.get("USR_ID") %>" >
			<A id="USR_IDHLP" name="USR_IDHLP"  href="#" onClick="getPicklistValues('g_pkl_userID','USR_ID','','')" class="linkblue" >?</A> &nbsp;
